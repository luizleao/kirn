...
class Form
{
    ...
    static function geraSelect($obj, $campo, $classeFK, $campoFK, $labelFK, $tipoTela, $gui) {
        switch ($tipoTela) {
            case 'CAD':
                ...
                break;
            case 'EDIT':
                $editValue = "\$o$classeFK"."->$campo == $obj"."->o$classeFK"."->$campoFK";
                break;
        }

        $retorno = Util::getConteudoTemplate($gui . '/Modelo.Form.Select.tpl');

        $retorno = str_replace('%%CLASSE%%',     $obj, $retorno);
            ...
            foreach ($aCampo[1] as $sCampo) {
                switch ($tipoTela) {
                    case 'CAD':
                        ...
                        break;
                    case 'EDIT':
                        $editValue = "<?=($obj" . "->$campo == \"$sCampo\") ? \" selected\" : \"\"?>";
                        break;
                }
    ...
    static function geraCalendario($obj, $campo, $label, $tipoTela, $dataHora, $gui)
    {
        try {
            ...
            switch ($tipoTela) {
                case 'ADM':
                    $retorno = ($dataHora) ? "Util::formataDataHoraBancoForm($obj" . "->$campo ?? NULL)" : "Util::formataDataBancoForm($obj" . "->$campo ?? NULL)";
                    break;
                case 'CAD':
                    $retorno = str_replace('%%CAMPO%%', $campo, $retorno);
                    $retorno = str_replace('%%LABEL%%', $label, $retorno);
                ...
                case 'EDIT':
                    ...
                    $retorno = str_replace('%%LABEL%%', $label, $retorno);
                    $retorno = str_replace('%%DATAHORA%%', ($dataHora) ? "true" : "false", $retorno);
                    $retorno = str_replace('%%VALOR%%', ($dataHora) ? "Util::formataDataHoraBancoForm($obj" . "->$campo ?? NULL)" : "Util::formataDataBancoForm($obj" . "->$campo ?? NULL)", $retorno);
                    break;
            }
            return $retorno;
        } catch (Exception $e) {
    ...
    /**
     * Gera um combobox com a lista de UFs
     *
     * @param string $obj Nome do Objeto
     * @param string $gui
     * @param string $tipoTela
     * @param string $campo
     * @param string $label
     * @param bool $dataHora
     * @return string|boolean|mixed
     */
    static function geraListaUf($obj, $gui, $tipoTela, $campo, $label, $dataHora)
    {
        ...
        switch ($tipoTela) {
            ...
            case 'EDIT':
                $retorno = str_replace('%%CAMPO%%', $campo, $retorno);
                $retorno = str_replace('%%LABEL%%', $label, $retorno);
                $retorno = str_replace('%%VALOR%%', "<?=$obj" . "->$campo ?? NULL ?>");
                break;
        }
        return $retorno;
    }
    ...
    static function geraCheckBox($obj, $campo, $label, $tipoTela, $gui)
    {
        switch ($tipoTela) {
            case 'CAD':
                ...
                break;
            case 'EDIT':
                $editValue = "<?=($obj" . "?->$campo == 1) ? ' checked=\"checked\"' : '' ?>";
                break;
        }
    ...
    {
        $retorno = "<input name=\"$campo\" type=\"hidden\" id=\"$campo\" value=\"<?=\$_REQUEST['$campo'] ?? NULL ?>\" />";
        return $retorno;
    }
